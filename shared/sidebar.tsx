...
import { Home, NotebookPen, ChevronLeft, User, Settings, ChevronDown, ChevronUp, LogOut } from "lucide-react";
...
const SideBar = () => {
  ...
  const getUserInitials = (fullName: string) => {
    if (!fullName) return <User className="w-5 h-5 text-gray-600 dark:text-gray-300" />;
                ...
                {item.path ? (
                  ...
                  <>
                    ...
                    {!sidebarCollapsed && feedbackExpanded && (
                      <div id="feedback-submenu" className="pl-12 space-y-1">
                        {item.subItems?.map((subItem) => (
                          <Link
                            key={subItem.path}
                            href={subItem.path}
                            className="flex items-center gap-2 px-3 py-2 rounded-lg text-black dark:text-gray-200 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
                          >
          ...
          {!sidebarCollapsed ? (
            <>
              ...
              {userMenuOpen && (
                <div 
                  id="user-menu"
                  className="absolute bottom-full left-0 right-0 mb-1 mx-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden z-10"
                >
